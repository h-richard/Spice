 ...
 * @author Hugo Richard
 */
public class SpiceValidator extends AbstractSpiceValidator {
	...
	public void checkOperation(XMod_Operation op) {
		...
		// format of each chain is tag:chainRoot.chainElement.chainElement.[...]
		EList<String> params = op.getParametersTag();
		for (String p : params)
			checkTag(p, root, SpicePackage.Literals.XMOD_OPERATION__PARAMETERS_TAG, params.indexOf(p));
		
		// checks the object chain
		// it follows the 'on' keyword, stating from which class the method belongs to
		checkTag(op.getObjectTag(), root, SpicePackage.Literals.XMOD_OPERATION__OBJECT_TAG, -1);
		
		// checks the optional return chain
		// it follows the 'returns' keyword
		if (op.getReturnTag() != null)
			checkTag(op.getReturnTag(), root, SpicePackage.Literals.XMOD_OPERATION__RETURN_TAG, -1);
	...
	/**
	 * Check the validity of a given tag chain.
	 * @param tagChain the said tag chain, usually formatted as tag:chainRoot.chainElement.chainElement.[...]
	 ...
	 */
	private void checkTag(String tagChain, EObject root, EAttribute literal, int featureIndex) {
		String[] couple = tagChain.split(":");
		if (couple.length < 2) return;
		Deque<String> chain = new ArrayDeque<>(Arrays.asList(couple[1].split("\\.")));
		switch (couple[0]) {
		case MODEL_TAG: {
			checkModelTag(root, chain, literal, featureIndex);
		}
		case EXTERNAL_TAG: {
			break;
		}
		...
		default: {
			if (featureIndex == -1) error("Unknown tag : '" + couple[0] + "'", literal);
			else error("Unknown tag : '" + couple[0] + "'", literal, featureIndex);
			break;
		}
		}
	}
	...
	/**
	 * Check specifically for tag chains starting with "model".
	 * @param root the model root class instance
	 * @param chain the tag chain split as a Deque (doesnt carry the tag)
	 * @param literal the meta object literal for the attribute represented by the tag chain
	 * @param featureIndex the index of the sub-literal if the attribute is a collection (or -1 if a regular attribute)
	 */
	private void checkModelTag(EObject root, Deque<String> chain, EAttribute literal, int featureIndex) {
		...
		if (chainRoot == null) {
			...
			System.out.println("---");
			for (EClassifier c : inferedTypeList) System.out.println(c);
		...
		// thus the whole chain must be valid
			...
			while (!chain.isEmpty()) {
				...
				// the current element doesnt contain brackets, it is a regular feature
		            ...
		            if (feature == null) {
		            	error("No field '"+fieldName+"' found.", literal, featureIndex);
		            	return;
		            }
	...
	private static List<EClass> getModelClasses(Predicate<EClass> filter) {
		EPackage pack = SpicePackage.eINSTANCE;
